...
signed main(signed argc, char const *argv[])
{
	...
	{
		cin>>u>>v;
		di.AddEdge(u,n+v,inf);
		di.AddEdge(v,n+u,inf);
	}
	for(int i=1;i<=n;i++)
		di.AddEdge(i,n+i,inf);
	for(int i=1;i<=n;i++)
	{
		di.AddEdge(st,i,a[i]);
		di.AddEdge(n+i,ed,b[i]);
	}
	if(di.Maxflow(st,ed)!=sum||s1!=sum)
		cout<<"NO"<<endl;
	else{
		cout<<"YES"<<endl;
