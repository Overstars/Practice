...
int main()
{
	...
	while(cin>>x1>>y1>>x2>>y2>>x3>>y3>>x4>>y4){
		...
		if(y3<y4)
			swap(y3,y4);
//		printf("1:%d 2:%d\n",cor(x1,x2,x3,x4),cor(y2,y1,y4,y3));
...
//			cout<<setiosflags(ios::fixed)<<setprecision(2)<<(fmin(x2-x3,x4-x1)*fmin(y3-y2,y1-y4))<<endl;
		...
		else if((cor(x1,x2,x3,x4)==2&&cor(y2,y1,y4,y3)==2)||(cor(x1,x2,x3,x4)==3&&cor(y2,y1,y4,y3)==3)){//完全在里面,输出较小的方块面积
//			cout<<setiosflags(ios::fixed)<<setprecision(2)<<fmin((x2-x1)*(y1-y2),(x4-x3)*(y3-y4))<<endl;
			printf("%.2lf\n",fmin((x2-x1)*(y1-y2),(x4-x3)*(y3-y4)));
		}
		else if(cor(x1,x2,x3,x4)==3&&cor(y2,y1,y4,y3)==2)
			cout<<setiosflags(ios::fixed)<<setprecision(2)<<(x2-x1)*(y3-y4)<<endl;
		else if(cor(x1,x2,x3,x4)==2&&cor(y2,y1,y4,y3)==3)
			cout<<setiosflags(ios::fixed)<<setprecision(2)<<(x4-x3)*(y1-y2)<<endl;
...
int cor(double a,double b,double c,double d)//前两而是一个正方形的平行边,从下到上，先左后右
{
	...
	if((a<c&&c<b&&b<=d)||(c<a&&a<d&&d<=b))
		return 1;
	else if((a<=c&&b>=d))//第二个在第一个中间
		return 2;
	else if((c<=a&&d>=b))//第一个在第二个中间
		return 3;
	return 0;//无交点
}
double fmin(double a,double b)
