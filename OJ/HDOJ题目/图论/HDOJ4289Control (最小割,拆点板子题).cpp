...
//        edges.push_back(Edge(to, from, 0, 0));
	...
	int DFS(int x, int a)
	{
		...
		if(!flow)
			dist[x] = -1;//炸点优化，必不可少，证明不必要的点下一次就不用遍历
		return flow;
	}
...
int main()
{
    ...
    while(cin>>n>>m)//点数,边数
    {
        ...
        for(int i=1;i<=n;i++)
		{
			int cost;
			cin>>cost;
			di.AddEdge(i,n+i,cost);//将点转化为边权
		}
		for(int i=1;i<=m;i++)
		{
			int a,b;
			cin>>a>>b;//道路为双向
			di.AddEdge(n+a,b,inf);//本题不可以去掉边
			di.AddEdge(n+b,a,inf);//在道路上必须畅通,容量为inf
		}//求的是通过城市的最大流
