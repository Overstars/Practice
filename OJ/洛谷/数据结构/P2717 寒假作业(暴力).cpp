...
//			ans++;
		st.insert(pre[i-1]);
		multiset<int>::iterator it=st.upper_bound(pre[i]),it2=st.begin();
		ll now=std::distance(it2,it);
//		ins(root,pre[i-1]);
//		int pp=getval(getrnk(pre[i])-1);
//		ll now=getrnk(inf)-1-getrnk(pre[i]+1)-1;
//		ll now=getrnk(getval(getrnk(pre[i])-1))-getrnk(-inf);//查询小于pre[i]的数量
//		printf("pp=%d,now=%lld\n",pp,now);
//		printf("%d-%d=%lld,pp=%d\n",getrnk(inf),getrnk(pp),now,pp);
		ans+=now;
	}
//	ldr(root);
	cout<<ans<<endl;
